...
function HomePage() {
  ...
  const planDayToWorkoutDay = (day: any): WorkoutDay => ({
    ...
    focus: day.focus,
    // Build three sections: WARM-UP (yellow), MAIN (default), COOL-DOWN
    // (teal). Saved exercises are grouped by their `kind` field; any
    // missing section gets an auto-picked suggestion from lib/stretching
    // so the user always sees a full warm-up + cool-down even if they
    // haven't customised them.
    sections: (() => {
      const all = day.exercises ?? [];
      const warmups = all.filter((ex: any) => ex.kind === "warmup");
      const cooldowns = all.filter((ex: any) => ex.kind === "cooldown");
      const mains = all.filter((ex: any) => !ex.kind || ex.kind === "main");

      const toSessionEx = (ex: any) => {
        const meta = (EXERCISES as any[]).find((e: any) => e.id === ex.exerciseId);
        return { id: ex.exerciseId, name: ex.name, sets: ex.sets, reps: ex.reps, rest: ex.rest, note: ex.notes ?? undefined, type: meta?.type ?? "compound", groupId: ex.groupId ?? undefined, groupType: ex.groupType ?? undefined, dropSets: ex.dropSets ?? 0, dropSet: ex.dropSet === true, kind: ex.kind ?? "main", trackable: ex.kind === "warmup" || ex.kind === "cooldown" ? false : undefined };
      };

      const stretchToSessionEx = (s: StretchExercise) => ({
        id: s.id, name: s.name, sets: 1, reps: s.reps,
        rest: 0, type: s.type, kind: s.kind, trackable: false,
        note: s.cues[0] ?? undefined, cues: s.cues, icon: s.icon, auto: true,
      });

      const warmupSection = warmups.length > 0
        ? { name: "Warm-Up", type: "warmup" as const, exercises: warmups.map(toSessionEx) }
        : { name: "Warm-Up", type: "warmup" as const, exercises: pickWarmups({ title: day.title, focus: day.focus }).map(stretchToSessionEx) };

      const mainSection = { name: "Main", type: "main" as const, exercises: mains.map(toSessionEx) };

      const cooldownSection = cooldowns.length > 0
        ? { name: "Cool-Down", type: "cooldown" as const, exercises: cooldowns.map(toSessionEx) }
        : { name: "Cool-Down", type: "cooldown" as const, exercises: pickCooldowns({ title: day.title, focus: day.focus }).map(stretchToSessionEx) };

      return [warmupSection, mainSection, cooldownSection];
    })(),
  });
                ...
                return (
                  <div key={ex.id} className="fade-in">
                    <div onClick={() => {
                      ...
                      {/* Compact row. Name on its own line, metadata (sets ×
                          reps · last session · status chips) on a single
                          dim line beneath, action buttons (FORM / EDIT /
                          DROP / SUPERSET) ONLY when the card is expanded
                          — collapsed cards are name + meta + set chips
                          so the list scans cleanly. The thumbnail itself
                          opens the FORM modal so FORM-while-collapsed
                          isn't lost. (qa: workout-set-logging) */}
                      {(() => {
                          ...
                          <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                            {(() => { const tu = getExerciseImageUrls(ex.id, ex.name); return tu ? (
                              <img src={tu[0]} alt="" onClick={e => { e.stopPropagation(); const m = lookupExMuscles(ex.name); setFormPreview({ id: ex.id, name: ex.name, ...m }); }}
                                style={{ width: 40, height: 40, borderRadius: 8, objectFit: "cover", flexShrink: 0, background: "#1a1a1a", cursor: "pointer" }}
                                onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} />
                            ) : null; })()}
                            <div style={{ flex: 1, minWidth: 0 }}>
                              {/* Name + completion marker. Name gets the
                                  full width minus a tiny ✓/TAP cluster. */}
                              ...
                              {/* Meta line — sets × reps, last session,
                                  and all status chips collapsed into one
                                  dim row. */}
                              <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginTop: 4, fontSize: 11, color: "rgba(255,255,255,0.4)", fontFamily: "'Space Mono', monospace" }}>
                                <span>{trackable ? `${ex.sets} × ${ex.reps}` : ex.reps}{ex.rest ? ` · ${ex.rest}s` : ""}</span>
                                {trackable && lw > 0 && <span style={{ color: "rgba(255,255,255,0.35)" }}>· last {lw}kg×{lr || "?"}</span>}
                                {ex.type && <span style={{ fontSize: 9, fontWeight: 600, color: bc[ex.type] || "#888", opacity: 0.85, letterSpacing: 1 }}>· {ex.type.toUpperCase()}</span>}
                                {exIsDropSet && <span style={{ fontSize: 9, fontWeight: 700, color: "#FFE66D", background: "rgba(255,230,109,0.12)", border: "1px solid rgba(255,230,109,0.25)", borderRadius: 4, padding: "0 5px", letterSpacing: 1 }}>🔻 DROP</span>}
                                {missing.length > 0 && (
                                  <button
                                    onClick={(e: any) => { e.stopPropagation(); setSubModal({ exerciseId: ex.id, name: ex.name, missing }); }}
                                    style={{ fontSize: 9, fontWeight: 700, color: "#FF8C42", background: "rgba(255,140,66,0.1)", border: "1px solid rgba(255,140,66,0.3)", borderRadius: 4, padding: "0 5px", letterSpacing: 1, fontFamily: "'Space Mono', monospace", cursor: "pointer" }}
                                ...
                                {hit.length > 0 && (
                                  <button
                                    ...
                                    style={{ fontSize: 9, fontWeight: 700, color: "#FF6B6B", background: "rgba(255,107,107,0.1)", border: "1px solid rgba(255,107,107,0.35)", borderRadius: 4, padding: "0 5px", letterSpacing: 1, fontFamily: "'Space Mono', monospace", cursor: "pointer" }}
                                  >🤕 {hit[0].muscle.toUpperCase()}</button>
                                )}
                                {ex.note === "HIIT circuit" && <span style={{ fontSize: 9, fontWeight: 700, color: "#FF8C42", background: "rgba(255,140,66,0.12)", border: "1px solid rgba(255,140,66,0.3)", borderRadius: 4, padding: "0 5px", letterSpacing: 1 }}>⚡ HIIT</span>}
                              </div>
                              {ex.note && ex.note !== "HIIT circuit" && <div style={{ fontSize: 11, color: "#f0c040", marginTop: 4, fontStyle: "italic", opacity: 0.85 }}>{ex.note}</div>}
                            </div>
                          </div>
                        );
                      })()}
                      {/* Action buttons appear ONLY when the card is
                          expanded — collapsed cards are clean. FORM
                          tap target is still on the thumbnail in
                          collapsed mode. */}
                      {isExp && (
                        <div style={{ display: "flex", alignItems: "center", gap: 4, flexWrap: "wrap", marginTop: 8, justifyContent: "flex-end" }}>
