...
> **2026-05-22 — placeholders shipped.** All 7 stretch demos
> (`cd-chest-doorway`, `cd-pigeon`, `cd-hamstring-lay`, `cd-lat-stretch`,
> `cd-glute-pretzel`, `wu-leg-swings`, `wu-scap-shrugs`) currently use
> text-card placeholder PNGs generated by
> `scripts/generate-stretch-placeholders.ts`. They alternate every 900ms
> so the user reads through START → STRETCH/END instructions. Replace
> any pair with real photos by overwriting `0.png` / `1.png` — no code
> change needed. Use the ChatGPT prompts below to generate the real
> photos when ready.

> **File-extension note.** Local demos are now `.png` (loader updated in
> `lib/exerciseImages.ts`). Free-exercise-db remote URLs stay `.jpg`.
